...
func updateProviderGo(resourceConfig tools.ResourceConfig) error {
	...
	pkgName := resourceConfig.PackageName
	importPath := fmt.Sprintf(`"github.com/scaleway/crossplane-provider-scaleway/config/%s"`, pkgName)

	if !strings.Contains(contentStr, importPath) {
		// Insert it into the import block
		// This regex finds the opening `import (` line
		re := regexp.MustCompile(`import\s*\(\s*`)
		contentStr = re.ReplaceAllString(contentStr, fmt.Sprintf("import (\n\t%s\n", importPath))
	}
